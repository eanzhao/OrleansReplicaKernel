...
│   └── Program.cs    # 端到端演示入口
├── test/             # 单元测试（60 个）
└── docs/             # 完整文档（81 篇，见 docs/README.md）
```
...
| **宿主** | Host 暴露当前 TimeProvider；提供 provider-aware delay/timeout/elapsed/wait helper；demo 里的状态观察开始脱离固定睡眠 |
...
2. **复刻设计文档**（38-81）：每个子系统在新架构下应该怎么实现
